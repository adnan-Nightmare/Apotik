  ...
  switch (action.type) {
    ...
    if (state.selectedCategory) {
      filtered = filtered.filter(
        (p) => p.kategori_id === Number(state.selectedCategory)
      );
    }
    ...
    if (state.searchTerm) {
      ...
      filtered = filtered.filter((p) =>
        (p.nama_obat && p.nama_obat.toLowerCase().includes(lowercasedTerm)) ||
        (p.nomor_batch && p.nomor_batch.toLowerCase().includes(lowercasedTerm))
      );
    }
